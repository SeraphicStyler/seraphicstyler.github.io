...
(() => {
  ...
  launcher.innerHTML = 'Menu <span aria-hidden="true">⌄</span>';
  ...
  dialog.innerHTML = `<div class="ss-menu-bokeh" aria-hidden="true"><i></i><i></i><i></i></div>
    ...
    <div class="ss-menu-tabs" role="group" aria-label="Menu view"><button type="button" data-view="explore" aria-pressed="true">Explore</button><button type="button" data-view="prices" aria-pressed="false">Services &amp; prices</button></div>
    <div data-explore><nav class="ss-menu-primary" aria-label="Primary"><a href="sourcingandstyling"><span>01</span>Find your service <small>Sourcing or styling?</small></a><a href="${sourcing}"><span>02</span>Sourcing <small>Purchase identified Vietnamese items</small></a><a href="${styling}"><span>03</span>Personal styling <small>Choose pieces that work for you</small></a><a href="#custom-wardrobe"><span>04</span>Custom Wardrobe <small>A fuller wardrobe, from $1,100</small></a></nav><nav class="ss-menu-secondary" aria-label="Explore more"><a href="${section('process', '#lp-how')}">How it works</a><a href="fashion-directory">Fashion directory</a><a href="${section('lookbook', 'index.html#lookbook')}">Lookbook</a><a href="${section('gift', '#lp-gift')}">Gifting</a><a href="about">About the atelier</a><a href="service-request.html">Start a request</a></nav></div>
    ...
    <section data-prices hidden><h3>Personal styling <span>USD</span></h3><p>Each booking includes a styling fee and clothing credit. Shipping is separate.</p><div class="ss-table-wrap" role="region" aria-label="Styling prices" tabindex="0"><table><thead><tr><th scope="col">Service</th><th scope="col">Booking</th><th scope="col">Fee</th><th scope="col">Clothes</th></tr></thead><tbody>${tiers.map((t,i) => `<tr><th scope="row"><a href="${i > 4 ? '#custom-wardrobe' : styling}">${t[0]}</a></th>${t.slice(1).map(v=>`<td>${v}</td>`).join('')}</tr>`).join('')}</tbody></table></div><p>Custom projects are quoted before booking. Clothing credit is a budget toward approved pieces; item counts depend on store prices.</p><h3>Item sourcing <span>VND</span></h3><p>Per item: 8% of store price (7% above 5,000,000₫), minimum 350,000₫. Coordination: 250,000₫ per order. The Trace: US$25 per item for identification research, credited toward your order. Alternatives require paid styling.</p><a href="links.html#lp-sourcing">Full sourcing fees &amp; payment costs →</a></section>
    <footer class="ss-menu-footer"><span>Thoughtfully chosen in Saigon. Sent worldwide.</span><button type="button" data-settings>Display &amp; accessibility</button><label><input type="checkbox" data-shortcuts> Enable O / M shortcuts</label><span>Esc to close</span></footer></div></div>`;
  ...
  consent.innerHTML = `<form method="dialog"><button class="ss-booking-close" value="cancel" formnovalidate aria-label="Close booking details">×</button><span class="eyebrow">Before you book · Personal styling</span><h2 id="ss-booking-title">Pieces chosen for you.</h2><p>This styling tier helps you decide what to buy. Outfit planning and ongoing support follow the scope of your selected tier. For a fuller capsule or multiple occasions, <a href="#custom-wardrobe" data-custom>explore Custom Wardrobe</a>.</p><p>The booking separates your styling fee from clothing credit. Shipping is additional, and any extra clothing budget is agreed before purchase.</p><label class="ss-consent"><input type="checkbox" required> I understand that sourcing purchases an identified, in-stock Vietnamese item without research. Alternatives, reference interpretation, outfits, and personal recommendations are paid styling; work begins after confirmation and payment.</label><button class="ss-action" value="continue">Continue to secure checkout ↗</button></form>`;
