...
(async () => {
 ...
 try {
  ...
  assert.equal(await page.evaluate(()=>document.activeElement.dataset.fields),'trace','redirect focus follows visible service');
  ...
  assert.equal(await page.evaluate(()=>document.activeElement.dataset.fields),'styling','larger research request focus');
